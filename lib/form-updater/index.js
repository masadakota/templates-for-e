/**
 * Form Updater
 * フォーム要素（checkbox, select, input等）の状態をターゲット要素のテキストに同期します
 *
 * 使用方法:
...
(function () {
  ...
  class FormUpdater {
    ...
    init(root = document) {
      ...
      console.log(
        `[FormUpdater] Initialized ${checkboxes.length} checkboxes, ${selects.length} selects, ${inputs.length} inputs`
      );
    }
    ...
    _setupElement(element) {
      ...
      if (targetClasses.length === 0) {
        console.warn("[FormUpdater] Missing target class:", element);
        return;
      }
    ...
    _updateTargets(targetClass, value) {
      ...
      if (targets.length === 0) {
        console.warn(
          `[FormUpdater] No targets found for class: ${targetClass}`
        );
      }
    }
    ...
    setElementValue(id, value) {
      ...
      if (element) {
        ...
        console.warn(`[FormUpdater] Element not found: ${id}`);
      }
    }
  ...
  // グローバルに公開
  window.FormUpdater = FormUpdater;
})();
