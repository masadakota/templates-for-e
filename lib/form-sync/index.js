 ...
 *
 * 使用方法:
 * <script src="../lib/form-sync/index.js"></script>
 * <script>
...
(function () {
  ...
  class FormSync {
    ...
    init(root = document) {
      ...
      console.log(
        `[FormSync] Initialized ${checkboxes.length} checkboxes, ${selects.length} selects, ${inputs.length} inputs`
      );
    }
    ...
    _setupElement(element) {
      ...
      if (targetClasses.length === 0) {
        console.warn("[FormSync] Missing target class:", element);
        return;
      }
    ...
    _updateTargets(targetClass, value) {
      ...
      if (targets.length === 0) {
        console.warn(
          `[FormSync] No targets found for class: ${targetClass}`
        );
      }
    }
    ...
    setElementValue(id, value) {
      ...
      if (element) {
        ...
        console.warn(`[FormSync] Element not found: ${id}`);
      }
    }
