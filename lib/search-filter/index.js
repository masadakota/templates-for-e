 ...
 *     debounceMs: 300  // 入力後の待機時間（ms）日本語入力に最適
 *   });
 * </script>
 *
 * または、デフォルト設定で初期化:
  ...
  class SearchFilter {
    constructor(options = {}) {
      const defaults = {
        ...
        debounceMs: 200, // 入力後の待機時間（ms）日本語入力に最適化
    ...
    _getDOMElements() {
      ...
      if (!this.searchInput) {
        throw new Error(
          `SearchFilter: Element with id "${this.options.searchInputId}" not found`,
        );
      }
    ...
    getHiddenItems() {
      return this.items.filter((item) =>
        item.classList.contains(this.options.hiddenClass),
      );
    }
    ...
    getVisibleItemsData() {
      return this.itemsData.filter(
        (data) => !data.element.classList.contains(this.options.hiddenClass),
      );
    }
