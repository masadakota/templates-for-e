 ...
 *        data-fb-priority="2" />
 * <span class="ohinichi"></span>
 * ※ 複数チェックされた場合、data-fb-priorityの値が小さい方（優先度が高い）が反映されます
 * ※ data-fb-priorityが未指定の場合は999として扱われます
 *
 * セレクトボックス:
 ...
 * テキスト入力（空文字を無視）:
 * <input type="text"
 *        data-fb-target=".name-text"
 *        data-fb-ignore-empty
 *        placeholder="名前を入力" />
 * <textarea data-fb-target=".comment-text"
 *           data-fb-ignore-empty="true"
 *           placeholder="コメント"></textarea>
 * <span class="name-text">未入力</span>
 * ※ data-fb-ignore-empty属性がある場合、空文字の時はターゲットを更新しません
 *
 * ラジオボタン:
 * <input type="radio"
 *        name="payment-type"
  ...
  class FormBinder {
    constructor(options = {}) {
      this.options = {
        ...
        valueSelector:
          "[value][data-fb-target]:not(input):not(select):not(option)",
        trueValueAttr: "data-fb-true-value",
        falseValueAttr: "data-fb-false-value",
        targetAttr: "data-fb-target",
        priorityAttr: "data-fb-priority",
        ignoreEmptyAttr: "data-fb-ignore-empty", // テキスト入力・textareaの空文字を無視するかどうか
    ...
    init(root = document) {
      ...
      const textareas = Array.from(
        root.querySelectorAll("textarea[data-fb-target]"),
      );
      ...
      console.log(
        `[FormBinder] 初期化完了: チェックボックス ${checkboxes.length}個, セレクト ${selects.length}個, 入力欄 ${inputs.length}個, テキストエリア ${textareas.length}個, value要素 ${valueElements.length}個`,
      );
    }
    ...
    _setupElement(element) {
      ...
      if (!targetSelector) {
        console.warn("[FormBinder] data-fb-target属性が見つかりません:", element);
        return;
      }
      ...
      const candidates = elementsWithSameTarget.map((element) => {
        ...
        if (element.tagName === "SELECT") {
          ...
          if (element.checked) {
            ...
            // チェックボックスがOFFの場合
            const falseValueAttr = element.getAttribute(
              this.options.falseValueAttr,
            );
            if (falseValueAttr !== null) {
              // data-fb-false-valueが設定されている場合はアクティブとして扱う
              isActive = true;
              value = falseValueAttr;
            } else {
              // data-fb-false-valueが省略されている場合は非アクティブ（元のテキストを保持）
              isActive = false;
              value = "";
            }
          }
          ...
          // テキスト入力、textareaなど
          value = element.value;
          // data-fb-ignore-empty属性の確認（true/false文字列）
          const ignoreEmpty = element.getAttribute(
            this.options.ignoreEmptyAttr,
          );
          const shouldIgnoreEmpty =
            ignoreEmpty === "true" || ignoreEmpty === "";

          if (shouldIgnoreEmpty) {
            // 空文字を無視する場合、空文字の時はアクティブにしない
            isActive = value !== "" && value !== null && value !== undefined;
          } else {
            // 空文字を無視しない場合、常にアクティブ
      ...
      try {
        ...
        targets.forEach((target) => {
          ...
          if (value === "[hidden]") {
            ...
            console.log(`[FormBinder] 要素を非表示にしました: ${targetSelector}`);
            return;
          }
        ...
        if (targets.length === 0) {
          console.warn(
            `[FormBinder] セレクタに該当するターゲットが見つかりません: ${targetSelector}`,
          );
        }
      } catch (error) {
        console.error(
          `[FormBinder] 無効なセレクタです: ${targetSelector}`,
          error,
        );
      }
    }
    ...
    setElementValue(id, value) {
      ...
      if (element) {
        ...
        console.warn(`[FormBinder] 要素が見つかりません: ${id}`);
      }
    }
