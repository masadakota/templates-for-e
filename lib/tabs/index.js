...
(function () {
  ...
  class Tabs {
     ...
     * @param {string|HTMLElement} [options.tabsContent] - TabsContentコンテナのセレクタまたは要素（省略時はtabsListと同じ）
     * @param {string} [options.defaultValue] - 初期表示するタブの値
     * @param {Function} [options.onValueChange] - タブ変更時のコールバック
     * @param {string} [options.triggerSelector] - TabsTriggerのセレクタ (default: '[data-tab]')
     * @param {string} [options.contentSelector] - TabsContentのセレクタ (default: '[data-tab-panel]')
    ...
    constructor(options = {}) {
      ...
      this.options = {
        ...
        defaultValue: null,
        onValueChange: null,
        triggerSelector: '[data-tab]',
        contentSelector: '[data-tab-panel]',
        activeClass: 'active',
        enableKeyboard: true,
        enableHistory: false,
        activeColor: null,
      ...
      // TabsListコンテナ要素を取得
      this.tabsList = typeof this.options.tabsList === 'string'
      ...
      // TabsContentコンテナを取得（未指定の場合はtabsListと同じ）
      if (this.options.tabsContent) {
        this.tabsContent = typeof this.options.tabsContent === 'string'
          ? document.querySelector(this.options.tabsContent)
          : this.options.tabsContent;
      } else {
        this.tabsContent = this.tabsList;
      }

      // トリガーとコンテンツの要素を取得
      this.triggers = Array.from(this.tabsList.querySelectorAll(this.options.triggerSelector));
      this.contents = Array.from(this.tabsContent.querySelectorAll(this.options.contentSelector));
    ...
    _init() {
      ...
      if (!initialValue && this.triggers.length > 0) {
        initialValue = this.triggers[0].getAttribute('data-tab');
      }
    ...
    _setupAccessibility() {
      ...
      this.contents.forEach((content) => {
        const value = content.getAttribute('data-tab-panel');
      ...
      // タブリストコンテナを探す
      const tabList = this.tabsList.querySelector('[role="tablist"]');
      if (tabList) {
    ...
    _getInitialValue() {
      ...
      // デフォルト値
      if (this.options.defaultValue && this._isValidValue(this.options.defaultValue)) {
     ...
     */
    _isValidValue(value) {
      return this.triggers.some(trigger => trigger.getAttribute('data-tab') === value);
    }
    ...
    _handleKeyDown(event) {
      ...
      if (nextIndex !== -1) {
        ...
        const nextValue = nextTrigger.getAttribute('data-tab');
    ...
    setValue(value, triggerCallback = true) {
      ...
      const activeTrigger = this.triggers.find(trigger => trigger.getAttribute('data-tab') === value);
      const activeContent = this.contents.find(content => content.getAttribute('data-tab-panel') === value);
      ...
      if (activeTrigger) {
        ...
        if (this.options.activeBorderColor) {
          activeTrigger.style.borderBottomColor = this.options.activeBorderColor;
        }
      }
    ...
    disableTrigger(value) {
      const trigger = this.triggers.find(t => t.getAttribute('data-tab') === value);
      if (trigger) {
    ...
    destroy() {
      ...
      if (this.options.enableKeyboard) {
        this.tabsList.removeEventListener('keydown', this.boundHandlers.keydown);
      }
