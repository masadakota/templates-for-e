...
(function () {
  ...
  class Tabs {
    ...
    constructor(options = {}) {
      ...
      // TabsContentコンテナを取得（document全体から検索）
      this.tabsContent = document;
      ...
      this.contents = Array.from(
        this.tabsContent.querySelectorAll(this.options.contentSelector),
      );
    ...
    _isValidValue(value) {
      return this.triggers.some(
        (trigger) => trigger.getAttribute("data-tab") === value,
      );
    }
    ...
    setValue(value, triggerCallback = true) {
      ...
      const activeContent = this.contents.find(
        (content) => content.getAttribute("data-tab-panel") === value,
      );
    ...
    disableTrigger(value) {
      const trigger = this.triggers.find(
        (t) => t.getAttribute("data-tab") === value,
      );
      if (trigger) {
